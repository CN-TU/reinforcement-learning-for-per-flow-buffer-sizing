...
Comparing the behavior of \gls{ours} (\autoref{fig:exampleBicOursNice}) with the behavior of Fifo (\autoref{fig:exampleBicFifo1000} and \autoref{fig:exampleBicFifo100}) for an example flow shows that while Fifo keeps a very large standing queue, \gls{ours} keeps the queue at the minimum that is possible to achieve maximum throughput and minimum queue according to our utility function (\autoref{eq:utility}). The comparison with FqCoDel (\autoref{fig:exampleBicFqCoDel}) shows that FqCoDel generally avoids a large standing queue but results in a very large spike in queued packets at the beginning of the flow. Thus, while CoDel was designed to avoid ``bufferbloat'' it actually induces a very large standing queue during the slow start phase of a flow. 
